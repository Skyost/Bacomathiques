...
\begin{document}
	...
	\begin{formula}[Relation signe de l'intégrale - signe de la fonction]
		...
		\begin{itemize}
			\item Si $f \gt 0$ sur $I$, alors $\displaystyle{\int_{a}^{b} f(x) \, \mathrm{d}x \gt 0}$.
			\item Si $f \lt 0$ sur $I$, alors $\displaystyle{\int_{a}^{b} f(x) \, \mathrm{d}x \lt 0}$.
			...
			\item Soit $g$ une fonction définie sur $I$ avec $f \gt g$ sur $I$, alors $\displaystyle{\int_{a}^{b} f(x) \, \mathrm{d}x \gt \int_{a}^{b} g(x) \, \mathrm{d}x}$.
		\end{itemize}
	\end{formula}
